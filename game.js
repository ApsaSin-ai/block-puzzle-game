...
function drawPreviews(){
  previews.forEach((ctxRef, idx)=>{
    ...
    if(!piece) return;
    const size = Math.floor(previewSize / 5); 
...
function draw(){
  ...
  if(currentPiece){
    ...
    drawPiece(ctx, currentPiece, shadowX, shadowY,"#ff7f50");
  }
  drawPreviews();
}
...
previews.forEach((ctxRef, idx)=>{
  ...
  canvasRef.addEventListener("touchstart",(e)=>{ e.preventDefault(); startDragPreview(e, idx); }, {passive:false});
});
...
canvas.addEventListener("touchstart",(e)=>{ e.preventDefault(); handleDrag(e); }, {passive:false});
canvas.addEventListener("touchmove",(e)=>{ e.preventDefault(); handleDrag(e); }, {passive:false});
